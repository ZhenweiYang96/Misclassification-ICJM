# knots for baseline hazard -----------------------------------------------
# get_knots <- function(nkn, Time) {
#   knts <- seq(0, max(Time), length.out = nkn + 2) #quantile(Time, probs = seq(0,1, length.out = nkn + 2), names = F)
#   knts <- tail(head(knts, -1), -1)
#   return(sort(c(rep(range(Time, 0), 4),knts)))
# }
